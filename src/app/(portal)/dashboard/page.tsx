...
function ChartSkeleton() {
  return (
    <div className="rounded-xl border bg-card p-6 shadow-sm space-y-4 animate-pulse">
      ...
      <div className="h-[280px] w-full bg-muted/50 rounded-lg flex items-center justify-center">
        <div className="text-xs text-muted-foreground animate-pulse">Loading signup telemetry...</div>
      </div>
    </div>
  );
}
...
// ==========================================
// Streamed Server Subcomponents
// ==========================================

async function DashboardStatsSection({ brandId }: { brandId: string }) {
  ...
  const [
    ...
    supabase
      .from("campaigns")
      .select("spend, reported_sent, reported_delivered")
      .eq("brand_id", brandId),
  ]);
  ...
  const contactabilityRate =
    totalCustomers && totalCustomers > 0 && contactableCustomers
...
async function DashboardCampaignsSection({
  brandId,
  brandName,
}: {
  brandId: string;
  ...
  const totalCampaigns = count ?? (campaigns?.length || 0);
    ...
    <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
      <div className="p-6 border-b flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Recent campaigns and engagement telemetry for {brandName}.
          </p>
        </div>
        <Link
          href="/campaigns"
...
// ==========================================
// Main Page Shell (Renders Immediately)
// ==========================================
...
export default async function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Header — Renders instantly with page shell */}
      ...
      {/* Streamed Hero Stats Section */}
      <Suspense fallback={<StatsSkeleton />}>
        <DashboardStatsSection brandId={brandId} />
      </Suspense>

      {/* Streamed Chart Section */}
      <Suspense fallback={<ChartSkeleton />}>
        <DashboardChartSection brandId={brandId} timezone={brand?.timezone} />
      </Suspense>

      {/* Methodology Explainer Note — Static, renders instantly */}
      ...
      {/* Streamed Recent Campaigns Table */}
      <Suspense fallback={<CampaignsTableSkeleton />}>
        <DashboardCampaignsSection brandId={brandId} brandName={brand?.name} />
      </Suspense>
    </div>
  );
}
